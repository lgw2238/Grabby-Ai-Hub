...
import com.lgw.grabby.domain.dto.*
import com.lgw.grabby.service.RagService
...
class RagController(private val ragService: RagService) {
    ...
    /**
     * METHOD : PDF 문서를 업로드하여 벡터 스토어에 저장
     */
    @Operation(
        summary = "PDF 문서 업로드",
        description = "PDF 파일을 업로드하여 벡터 스토어에 저장"
    )
    @SwaggerResponse(
        responseCode = "200",
    ...
    @SwaggerResponse(responseCode = "500", description = "아마도 ? 서버 오류")
    @PostMapping("/documents", consumes = [MediaType.MULTIPART_FORM_DATA_VALUE])
    fun uploadDocument(
        @Parameter(description = "IN UPLOAD PDF FILES", required = true)
    ...
    /**
     * 사용자 질의에 대해 관련 문서를 검색하고 RAG 기반 응답을 생성한다.
     */
    @Operation(
        summary = "RAG 질의 수행",
